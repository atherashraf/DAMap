    ...
    displayFeatureInfo(evt, mapVm, targetElem) {
        ...
        if (features.length > 0) {
            ...
            if (gType === 'Polygon' && feature.flatCoordinates_) {
                const inflatedCoordinates = inflateCoordinatesArray(feature.getFlatCoordinates(), // flat coordinates
                0, // offset
    ...
    showAreaInRightDraw(arrData, targetElem) {
        ...
        document.getElementById("btnShowChart").onclick = function (e) {
            me.mapVm.getDialogBoxRef().current.openDialog({
                "title": "Area Chart",
                "content": React.createElement("div", { style: { width: 600 } },
                    React.createElement(DAChart, { chartData: data })),
                "actions": React.createElement("p", null)
            });
        };
    }
}
