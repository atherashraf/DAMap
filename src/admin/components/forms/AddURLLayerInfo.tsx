...
const AddURLLayerInfo = (props: IProps) => {
    ...
    const [layerCategoryID, setLayerCategoryID] = React.useState<ILayerCategory>()
    ...
    const getForm = () => {
        switch (formType) {
            case "LayerCategory":
                return <AddLayerCategoryForm key={"LayerCategoryFormKey"} snackbarRef={props.snackbarRef}
                                             handleBack={() => setFormType(null)}/>
        }
    }

    const handleAddLayerInfo=()=>{
        mapApi.get(MapAPIs.DCH_ADD_URL_LAYER_INFO, {
            layer_title: layerTitle,
            layer_category_id: layerCategoryID,
            layer_url: layerURL,
            url_type: selectedItem}).then(()=>{
            ...
            {formType !== null ? getForm() :
                <Container maxWidth="lg" sx={{p: 3}}>
                    ...
                    <Box sx={{margin: "30px"}}>
                        ...
                        <TextField id="filled-basic" label="Layer Title" variant="filled"
                                   fullWidth={true} onChange={(e)=> setLayerTitle(e.target.value as string)}/>
                    </Box>
                    <Box sx={{margin: "30px"}}>
                        <LayerCategoryControl api={mapApi} setLayerCategory={
                            (layerCategory: ILayerCategory) => setLayerCategoryID(layerCategory)}
                                              handleAddLayerCategory={() => setFormType("LayerCategory")}/>
                    </Box>
                    <Box sx={{margin: "30px"}}>

                        <TextField id="filled-basic" label="Add URL" variant="filled"
                                   fullWidth={true} onChange={(e)=> setLayerURL(e.target.value as string)}/>
                    </Box>
                    <Box sx={{margin: "30px"}}>
