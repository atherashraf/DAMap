  ...
  const handleOnChange = (e: any) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const index: number = rows.findIndex(
        //@ts-ignore
        (item) => item.name === fileParts[0]
      );
      const row: any =
        index === -1 ? { name: fileParts[0], files: [] } : rows[index];
      row.files.push(file);
      switch (fileParts[1]) {
        case "shp":
          row["shp"] = 1;
    ...
    api.postFormData(MapAPIs.DCH_UPLOAD_SHP_FILE, formData).then((response) => {
      ...
      if (response) {
        ...
        props.snackbarRef.current?.show(
          "Failed to upload shape file... Please check with admin."
        );
        setDisableUpload(false);
      }
    });
                  ...
                  {rows.map((row, index) => (
                    <TableRow key={row.name}>
                      ...
                      <TableCell align="right">
                        {row.shp === 1 ? <CheckRounded /> : <ClearRounded />}
                      </TableCell>
                      <TableCell align="right">
                        {row.shx === 1 ? <CheckRounded /> : <ClearRounded />}
                      </TableCell>
                      <TableCell align="right">
                        {row.dbf === 1 ? <CheckRounded /> : <ClearRounded />}
                      </TableCell>
                      <TableCell align="right">
                        {row.prj === 1 ? <CheckRounded /> : <ClearRounded />}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
