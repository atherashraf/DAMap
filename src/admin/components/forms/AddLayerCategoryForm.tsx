        ...
        <React.Fragment>
            ...
            >
                ...
                <form onSubmit={handleSubmit}>
                    <Box sx={{margin: "30px"}}>
                        <TextField id="main-category" label="Main Category" variant="standard" fullWidth={true}
                                   value={mainCategory}
                                   onChange={(e) => setMainCategory(e.target.value as string)}
                                   required={true} error={mainCategory === ""}
                        />
                    </Box>
                    <Box sx={{margin: "30px"}}>
                        <TextField id="sub-category" label="Sub Category" variant="standard" fullWidth={true}
                                   value={subCategory}
                                   onChange={(e) => setSubCategory(e.target.value as string)}
                                   required={true} error={subCategory === ""}
                        />
                    </Box>

                    <Box sx={{margin: "30px"}}>
                        <Button type="submit" sx={{backgroundColor: "black", color: "white"}} variant="contained">
                            Create Category
                        </Button>
                        &nbsp; &nbsp;
