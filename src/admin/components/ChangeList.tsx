...
class ChangeList extends React.PureComponent<ICLGridProps, IChangeListState> {
    private jqxGrid: React.RefObject<JqxGrid> = React.createRef<JqxGrid>();
        ...
        const source: any = {
            ...
            updaterow: (rowid: number, rowdata: any, commit: any): void => {
                const editedData: IEditedData = {
                    ...
                    pk: {
                        colName: this.props.pkColName,
                        colValue: rowdata[this.props.pkColName],
                    },
                    rowData: rowdata,
                };
                this.props.api
                    .post(MapAPIs.DCH_EDIT_MODEL_ROW, editedData, {
                        modelName: this.props.modelName,
                    })
                    .then((payload) => {
                        if (payload) {
                            this.props.api.snackbarRef?.current?.show(
    ...
    getToolbarContainer() {
        ...
        >
          ...
          <select
              onChange={(e) => {
                  // @ts-ignore
                  this.selectedAction = this.state.actions.find(
    ...
    startEditing() {
        ...
        this.props.api?.snackbarRef?.current?.show(
            "Please double click on cell for editing..."
        );
    }
