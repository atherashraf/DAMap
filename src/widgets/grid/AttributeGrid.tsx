...
> {
  ...
  handleRowSelect() {
    ...
    if (!row["geom"]) {
      ...
      this.props.mapVM
        .getApi()
        .get(MapAPIs.DCH_GET_FEATURE_GEOMETRY, {
      ...
      },
      {
        ...
        onClick: () => {
          ...
          if (this.state.pivotTableData.length > 0) {
            this.dialogRef.current?.setContent(
              "Pivot Table",
              <PivotTable data={this.state.pivotTableData} />
            );
          } else {
  ...
  renderToolbar(toolbar: any) {
    if (!this.state.isToolbarRendered) {
      ...
      root.render(
        <React.Fragment>
          <AttributeGridToolbar
            ref={this.toolbarRef}
            mapVM={this.props.mapVM}
            daGrid={this.jqxGridRef}
          />
        </React.Fragment>
      );
