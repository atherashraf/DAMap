...
class SingleBandStyling extends React.PureComponent<IProps, IState> {
    ...
    calculateClassValue(classIndex: number) {
        ...
        if (percentile >= 0 && percentile <= 25) {
            ...
            value = bandInfo.q25 + (bandInfo.median - bandInfo.q25) * (percentile - 25) / 25
        } else if (percentile >= 50 && percentile <= 75) {
            value = bandInfo.median + (bandInfo.q75 - bandInfo.median) * (percentile - 50) / 25
        } else {
            value = bandInfo.q75 + (bandInfo.max - bandInfo.q75) * (percentile - 75) / 25
        }
        return value
    }
    ...
    saveStyle() {
        ...
        this.props.mapVM.getApi().post(MapAPIs.DCH_SAVE_STYLE, res,
                ...
                <Box sx={{flex: 1, pt: 1}}>
                    <FormControl fullWidth size={"small"}>
                        ...
                        <TextField type={"number"} value={this.state.noOfClasses}
                                   ...
                                   size={"small"}
                                   onChange={(e) =>
                                       this.setState({noOfClasses: parseInt(e.target.value as string || "0")})}
                                   InputProps={{inputMode: 'numeric', inputProps: {min: 1, max: 10}}}/>
                    </FormControl>
                </Box>
