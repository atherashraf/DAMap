...
class LegendGridJqxToolbar {
    ...
    public buttons = [
        {id: "addButton", value:"Add", onClick: this.addButtonOnClick.bind(this)},
        {id: "deleteButton", value: "Delete", onClick: this.deleteButtonOnClick.bind(this)},
    ]
    ...
    renderToolbar(toolbar: any) {
        ...
        const buttonsContainer = (
            <div style={{overflow: 'hidden', position: 'relative', margin: '5px'}}>
                {this.buttons.map((item)=> <div id={item?.id} key={item.id} style={style}/>)}
            </div>
        );
