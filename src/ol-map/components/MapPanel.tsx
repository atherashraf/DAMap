...
class MapPanel extends React.PureComponent<IProps, IState> {
  ...
  openBottomDrawer(
    height: number,
    contents: JSX.Element = <React.Fragment></React.Fragment>
  ) {
  ...
  resizeDrawer(height: number) {
    ...
    const mapDivHeight =
      height === 0 || totalMapHeight < height
        ? totalMapHeight + "px"
        : totalMapHeight - height + "px";
  ...
  getPaperHeight() {
    return this.state.totalMapHeight <= this.maxMapHeight
      ? this.state.drawerHeight + this.state.totalMapHeight
      : this.state.totalMapHeight;
  }
